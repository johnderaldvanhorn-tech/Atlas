...
function showView(v){$$('.view').forEach(x=>x.classList.add('hidden'));const view=$('#'+v+'View');if(!view)return;view.classList.remove('hidden');$$('nav button').forEach(b=>b.classList.toggle('active',b.dataset.view===v));const labels={dashboard:['Fiscal Year Product Initiatives','Evaluate, cost, prioritize, and schedule engineering initiatives.'],projects:['Product Initiatives','Review and edit the complete project pipeline.'],roadmap:['Initiative Roadmap','Review calculated project timing, milestones, and dependencies.'],development:['Development Support','Track bugs, enhancements, tasks, releases, and product support work.'],resources:['Resource Capacity','Manage labor rates, monthly capacity, and project allocations.'],reports:['Portfolio Reports','Review performance by fiscal year and quarter.'],scorecard:['Scorecard','Maintain executive metrics used by reports and board updates.'],settings:['Settings','Configure persistence and formula defaults.']};const label=labels[v]||[v,''];$('#pageTitle').textContent=label[0];$('#pageSubtitle').textContent=label[1];$('#newProject').classList.toggle('hidden',['settings','resources','development','scorecard','reports'].includes(v));window.currentPortfolioView=v;if(v==='scorecard')window.renderScorecard?.();window.refreshAllData?.({reason:`view:${v}`});}
